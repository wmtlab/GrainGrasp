...
if __name__ == "__main__":
    ...
    # obj_pc can be sampled from the obj_mesh, but need cvae have more ability to generalize
    obj_pc = tools.pc_sample(obj_mesh, sample_points_num)
    # obj_pc can be also loaded from the file
    obj_pc_path = os.path.join("sample", str(args.idx), "obj_pc.npy")  # [3,3000]
    obj_pc = np.load(obj_pc_path).T
